...
class Objective(Evaluator):
    ...
    def get_new_config(self, trial):
        ...
        if self.opt.tracking_method == 'ocsort':
            ...
            # trial.suggest_categorical("use_byte", [True, False])
            use_byte = False
            ...
            # override
            self.opt.conf = 0.424880788280455

            det_thresh = 0  # trial.suggest_int("det_thresh", 0.3, 0.6)
            max_age = 50  # trial.suggest_int("max_age", 10, 60, step=10)
            min_hits = 1  # trial.suggest_int("min_hits", 1, 5, step=1)
            # trial.suggest_float("iou_thresh", 0.1, 0.4)
            iou_thresh = 0.2625783256595632
            delta_t = 1  # trial.suggest_int("delta_t", 1, 5, step=1)
            # trial.suggest_categorical("asso_func", ['iou', 'giou'])
            asso_func = 'giou'
            # trial.suggest_float("inertia", 0.1, 0.4)
            inertia = 0.3348497441538068
            w_association_emb = trial.suggest_float(
                "w_association_emb", 0.2, 0.9)
            alpha_fixed_emb = trial.suggest_float(
                "alpha_fixed_emb", 0.9, 0.999)
            aw_param = trial.suggest_float("aw_param", 0.3, 0.7)
            # trial.suggest_categorical("embedding_off", [True, False])
            embedding_off = False
            # trial.suggest_categorical("cmc_off", [True, False])
            cmc_off = True
            aw_off = True  # trial.suggest_categorical("aw_off", [True, False])
            # trial.suggest_categorical("new_kf_off", [True, False])
            new_kf_off = True
        ...
        elif self.opt.tracking_method == 'efficientocsort':
            # override
            self.opt.conf = 0.424880788280455
            ...
            alpha_fixed_emb = trial.suggest_float(
                "alpha_fixed_emb", 0.9, 0.999)
            aw_param = trial.suggest_float("aw_param", 0.3, 0.7)
            embedding_off = trial.suggest_categorical(
                "embedding_off", [True, False])
            # trial.suggest_categorical("cmc_off", [True, False])
            cmc_off = True
            aw_off = True  # trial.suggest_categorical("aw_off", [True, False])
            # trial.suggest_categorical("new_kf_off", [True, False])
    ...
    def __call__(self, trial):
        ...
        print("RUnning the trial with below")
...
def save_plots(opt, study, objectives):
    ...
    for i, ob in enumerate(objectives):
        if not opt.n_trials <= 1:  # more than one trial needed for parameter importance
            ...
            fig.write_html(f"{ob}_param_importances_" +
                           opt.tracking_method + ".html")
...
def parse_opt():
    ...
    parser.add_argument('--reid-model', type=str,
                        default=WEIGHTS / 'lmbn_n_cuhk03_d.pt')
    parser.add_argument('--tracking-method', type=str,
                        default='deepocsort', help='strongsort, ocsort')
    parser.add_argument('--name', default='exp',
                        help='save results to project/name')
    parser.add_argument('--project', default=ROOT / 'runs' /
                        'evolve', help='save results to project/name')
    parser.add_argument('--classes', nargs='+', type=str,
                        default=['0'], help='filter by class: --classes 0, or --classes 0 2 3')
    ...
    opt.tracking_config = ROOT / 'boxmot' / opt.tracking_method / \
...
if __name__ == "__main__":
    opt = parse_opt()
    check_requirements(
        ('optuna', 'plotly', 'kaleido', 'joblib', 'pycocotools'))
    import joblib
    import optuna
    ...
    if opt.resume:
        ...
        study = optuna.create_study(
            directions=['maximize']*len(opt.objectives))
    ...
    study.optimize(Objective(opt), n_trials=opt.n_trials,
                   callbacks=[continuous_study_save_cb])
